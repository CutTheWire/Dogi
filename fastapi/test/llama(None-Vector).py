'''
파일은 LlamaOfficeModel, base_config.OfficePrompt 클래스를 정의하고 llama_cpp_cuda를 사용하여,
...
from src.service import vector_client
...
from src.domain import base_config
...
def build_llama3_prompt_with_rag(character_info: base_config.OfficePrompt, vector_context: str = "") -> str:
    """
    RAG 컨텍스트를 포함한 Llama3 GGUF 형식의 프롬프트 문자열을 생성합니다.

    Args:
        character_info (base_config.OfficePrompt): 캐릭터 기본 정보 및 대화 맥락 포함 객체
...
def build_llama3_prompt(character_info: base_config.OfficePrompt) -> str:
    """
...
class LlamaOfficeModel:
    ...
    def __init__(self, enable_rag: bool = True) -> None:
        ...
        self.character_info: Optional[base_config.OfficePrompt] = None
        self.config: Optional[base_config.LlamaGenerationConfig] = None
        ...
        if self.enable_rag:
            ...
            try:
                self.vector_search = vector_client.VectorSearchClient()
    ...
    def _stream_completion(self, config: base_config.LlamaGenerationConfig) -> None:
        """
        별도 스레드에서 실행되어 응답을 큐에 넣는 메서드 (최적화)

        Args:
            config (base_config.LlamaGenerationConfig): 생성 파라미터 객체
        """
        try:
    ...
    def create_streaming_completion(self, config: base_config.LlamaGenerationConfig) -> Generator[str, None, None]:
        """
        스트리밍 방식으로 텍스트 응답 생성
    ...
    def create_completion(self, config: base_config.LlamaGenerationConfig) -> str:
        """
    ...
    def generate_response(self, input_text: str, search_text: str, chat_list: List[Dict]) -> str:
        ...
        try:
            ...
            # 캐릭터 정보 설정
            self.character_info = base_config.OfficePrompt(
            ...
            # 균형 잡힌 설정으로 수정
            self.config = base_config.LlamaGenerationConfig(
                prompt = prompt,
...
if __name__ == "__main__":
    ...
    def run_comprehensive_test():
        """포괄적인 테스트 실행 및 결과 저장"""
        logger.info("=== 기본 모델 테스트 시작 (벡터 DB 비활성화) ===")
        ...
        try:
            ...
            # 모델 초기화 (RAG 비활성화)
            model = LlamaOfficeModel(enable_rag=False)
            ...
            filename = f"basic_model_test_results_{timestamp}.md"
    ...
    def generate_markdown_report(test_results, model):
        ...
        md_content = f"""# 🐕 반려견 AI 상담 시스템 테스트 결과 (벡터 DB 비활성화)
...
- **RAG 시스템**: {rag_status}
- **모델**: {model.model_id}
- **테스트 모드**: 기본 모델 (벡터 검색 없음)

## 🔧 시스템 설정
- **벡터 DB 연결**: {vector_status.get('status', 'DISABLED')}
- **문서 수**: N/A (벡터 DB 비활성화)
- **사용 가능한 진료과**: N/A (벡터 DB 비활성화)
...
### 모델 설정
- **벡터 검색**: 비활성화
- **컨텍스트**: 시간 정보 및 기본 프롬프트만 사용
- **응답 방식**: 모델 자체 지식 기반

### 시스템 권장사항
1. **응답 품질**: 기본 모델 지식에 의존, 일반적인 의료 조언 제공
2. **응답 속도**: 평균 {avg_response_time:.2f}초로 실용적 수준
3. **정확성**: 전문의 상담 권유 등 의료 안전성 고려
4. **한계점**: 특정 의료 데이터베이스 정보 없이 일반적 답변만 가능

### 벡터 DB 활성화 시 기대 효과
- 더 정확하고 구체적인 의료 정보 제공
- 실제 의료 케이스 기반 답변
- 진료과별 전문적인 조언
...
*🕒 보고서 생성 시간: {timestamp}*  
*🤖 Generated by Basic Llama AI System (No Vector DB)*
"""
