...
REM 필수 환경변수 로드 - 경로 수정
for /f "usebackq tokens=1,2 delims==" %%a in ("./fastapi/src/.env") do (
    set %%a=%%b
)
...
if /i "%libs_choice%"=="y" (
    ...
    docker volume rm dogi_python-libs 2>nul
...
echo [INFO] dogi-base 베이스 이미지 빌드...
docker build -f fastapi/src/Dockerfile.base -t dogi-base:latest .
if errorlevel 1 (
    echo [ERROR] dogi-base 빌드 실패. 종료합니다.
    exit /b 1
)
echo.
...
echo [INFO] 서비스 접속 정보:
echo [INFO] - Server: http://localhost
